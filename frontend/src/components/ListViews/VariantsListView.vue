<template>
  <ListView
    ...
    :rows="rows"
    :options="{
      getRowRoute: (row) => ({ name: 'Variant', params: { variantId: row.name } }),
  ...
  >
    ...
    <ListRows id="list-rows">
      ...
      >
        ...
        <div
          v-if="column.key === 'variant_code'"
          class="flex items-center"
          @click="(event) => emit('applyFilter', { event, idx, column, item })"
        >
        <Tooltip :text="item.label">
              <div>{{ row.variant_code }}</div>
            </Tooltip>
        </div>
        <ListRowItem
          v-else
        ...
        >
          <template #prefix>
            <div v-if="column.key === 'status'">
              <div v-if="!row.disabled" class="text-blue-500 bg-blue-100 rounded-md text-sm font-medium px-2 py-1 w-20 text-center">Enabled</div>
