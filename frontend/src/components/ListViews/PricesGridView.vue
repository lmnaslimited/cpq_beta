<template>
    <GridView
      ...
      row-key="name"
    >
      <GridHeader class="mx-5" @columnWidthUpdated="emit('columnWidthUpdated')" />
      <GridCards id="grid-cards" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-5">
        <GridCard
          v-for="row in rows"
          :key="row.name"
...
onMounted(() => {
  if (!grid.value?.data) return
  setupListActions(grid.value.data, {
    grid: grid.value,
    call,
    createToast,
    $dialog,
...
console.log(grid.value.data)
  customBulkActions.value = grid.value?.data?.bulkActions || []
  customListActions.value = grid.value?.data?.listActions || []
})
