<template>
  <LayoutHeader v-if="quotation.data">
    ...
    <template #right-header>
      <CustomActions
        v-if="quotation.data._customActions"
      ...
      <Dropdown :options="statusOptions('quotation', updateField)">
        <template #default="{ open }">
          <Button
            :label="quotation.data.status"
            :class="getQuotationStatus(Quotation.data.status).colorClass"
          >
            <template #prefix>
              <IndicatorIcon />
            </template>
          ...
          <div class="flex items-center justify-start gap-5 border-b p-5">
            <div class="group relative h-[88px] w-[88px]">
              <Avatar
                size="3xl"
                class="h-[88px] w-[88px]"
                :label="quotation.data.name"
                :image="quotation.data.image"
              />
              <component
                :is="quotation.data.image ? Dropdown : 'div'"
                v-bind="
                  quotation.data.image
                    ? {
                        options: [
                          {
                            icon: 'upload',
                            label: quotation.data.image
                              ? __('Change image')
                              : __('Upload image'),
            ...
            <div class="flex flex-col gap-2.5 truncate">
              <Tooltip :text="quotation.data.name">
...
function validateRequired(fieldname, value) {
  let meta = quotation.data.all_fields || {}
