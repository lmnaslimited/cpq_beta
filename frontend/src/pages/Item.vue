<template>
    <LayoutHeader v-if="item.data">
      ...
      <template #right-header>
        <CustomActions
          v-if="item.data._customActions"
          :actions="item.data._customActions"
        />
        <component :is="item.data._assignedTo?.length == 1 ? 'Button' : 'div'">
          <MultipleAvatar
            :avatars="item.data._assignedTo"
            @click="showAssignmentModal = true"
          />
        </component>
        <Dropdown :options="statusOptions('item', updateField)">
          <template #default="{ open }">
            <Button
              :label="item.data.status"
              :class="getItemStatus(item.data.status).colorClass"
            >
              <template #prefix>
                <IndicatorIcon />
              </template>
